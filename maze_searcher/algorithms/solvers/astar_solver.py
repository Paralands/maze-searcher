...
class ASTARSolver(SolverBase):
    ...
    def __init__(self, grid: np.ndarray, start: tuple[int, int], end: tuple[int, int]):
        ...
        Args:
            grid (np.ndarray): The maze grid to solve.
    ...
    def _heuristic(self, a: tuple[int, int], b: tuple[int, int]) -> int:
        """
        Calculate the Manhattan distance _heuristic between two points.
    ...
    def find_path(self) -> Iterator[np.ndarray]:
        ...
        """
        open_set = []
        heapq.heappush(open_set, (self._heuristic(self.start, self.end), 0, self.start))
        came_from = {}
        ...
        while open_set:
            ...
            for neighbor in self._neighbors(current):
                ...
                if neighbor not in g_score or tentative_g < g_score[neighbor]:
                    ...
                    f_score = tentative_g + self._heuristic(neighbor, self.end)
